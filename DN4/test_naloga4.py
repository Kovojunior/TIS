...
def test_naloga4(case_dir, case_id):
    ...
    # Preberemo sliko
    slika_file = Image.open(base_path / str(case_dir) / data['slika']).convert('L') # read as grayscale
    slika = np.array(slika_file, dtype=np.int16)

    # Poženemo rešitev domače naloge in izmerimo izvajalni čas
    start = timer()
    PSNR = naloga4(slika, data['velikostOkna'])
    end = timer()
    t_elapsed = end - start

    # Ovrednotimo rešitev
    success = abs(PSNR - data['PSNR']) < tol
    ...
    if success != 1:
        print(' ✗ Napačen `PSNR`')
        print(f'   -> Izracunani PSNR: "{PSNR}"')
        print(f'   -> Pravi PSNR: "{data["PSNR"]}"')
